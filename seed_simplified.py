"""Simplified database seeding script with cleaner URLs.

This module populates the database with initial company information
using simplified URLs that are more compatible with web scraping.
"""

from database import SessionLocal
from models import CompanySQL

# Simplified URLs without complex query parameters
SIMPLIFIED_SITES = {
...
# Keep filtered URLs as backup if simple ones don't work
FILTERED_SITES = {
...
def seed_companies(use_filtered: bool = False) -> None:
    """Seed the database with initial company data.

    Args:
        use_filtered: If True, use complex filtered URLs. If False, use simplified URLs.
    ...
    """
    sites = FILTERED_SITES if use_filtered else SIMPLIFIED_SITES
    ...
    try:
        for name, url in sites.items():
            ...
            if company:
                ...
                if company.url != url:
                    print(
                        f"Updating {name} URL: {company.url[:50]}... -> {url[:50]}..."
                    )
                    company.url = url
            else:
                # Add new company
        ...
        session.commit()
        url_type = "filtered" if use_filtered else "simplified"
        print(f"✅ Company database updated successfully with {url_type} URLs.")
...
if __name__ == "__main__":
    ...
    print(f"Using {'filtered' if use_filtered else 'simplified'} URLs...")
    seed_companies(use_filtered=use_filtered)
