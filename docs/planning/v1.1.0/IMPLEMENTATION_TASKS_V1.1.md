...
### **T2.4: Enhance Real-Time Progress Dashboard**
...
- **Libraries**: `streamlit==1.47.1`
- **Description**: Upgrade the V1.0 text-based progress dashboard to a rich, card-based layout with calculated metrics for speed and ETA.

- **Sub-tasks & Instructions**:
  - **T2.4.1: Update Progress Data Structures & Utilities**:
    - **Instructions**: In your `AppState` or progress data structure, ensure you are storing the `start_time` for each company's scraping task.
  ...
  - **T2.4.2: Create the Company Progress Card Component**:
  ...
  - **T2.4.3: Upgrade the Scraping Page UI**:
    - **Instructions**: In `src/ui/pages/scraping.py`, replace the V1.0 `for` loop that rendered `st.text`.
    - **Instructions**: Instead, use `st.columns` to create a responsive grid. In the loop, instantiate and render your new `CompanyProgressCard` for each company.
    - **Instructions**: Above the grid, add `st.metric` displays for the overall "ETA" and "Total Jobs Found".
    - **Success Criteria**: The scraping page now displays a grid of professional-looking progress cards. The metrics for speed and ETA update in real-time during a scraping session.
