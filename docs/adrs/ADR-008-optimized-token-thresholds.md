# ADR-008: Optimized Token Thresholds for Hybrid Processing
...
**Decided** - Updates ADR-006 Hybrid Strategy
...
- IR-034: Seamless integration with ADR-006 hybrid strategy
...
- **Updates ADR-006:** Hybrid LLM Strategy (implements new threshold)
- **Impacts ADR-004:** Local AI Integration (utilizes full model capabilities)
- **Connects to ADR-005:** Inference Stack (enables larger context processing)
...
```python
# Integration with ADR-006 SimpleHybridStrategy
...
- Integrated with ADR-006 hybrid strategy
