...
class TestApiConnection:
    ...
    def test_openai_invalid_format_returns_error(self):
        ...
        success, message = test_api_connection("OpenAI", "invalid-key")
        assert not success
        assert message == "Invalid OpenAI API key format (should start with 'sk-')"

    def test_groq_short_key_returns_error(self):
        """Test Groq API key length validation."""
        success, message = test_api_connection("Groq", "short")
        assert not success
        assert message == "Groq API key appears to be too short"
    ...
    @mock.patch.object(Groq, "chat")
    def test_groq_successful_connection(self, mock_chat):
        """Test successful Groq API connection."""
        # Mock completion response
        mock_completion = mock.Mock()
        mock_completion.id = "chat-12345678"
    ...
    @mock.patch.object(Groq, "chat")
    def test_groq_successful_connection_no_id(self, mock_chat):
        """Test successful Groq connection when completion has no ID."""
        # Mock completion response without ID
        mock_completion = mock.Mock()
    ...
    def test_unknown_provider_returns_error(self):
        ...
        assert not success
        assert message == "Unknown provider: UnknownProvider"
    ...
    @mock.patch.object(Groq, "chat")
    def test_groq_authentication_error(self, mock_chat):
...
class TestSettingsLoadSave:
    ...
    def test_save_settings_updates_session_state(self):
        ...
        assert st.session_state["llm_provider"] == "Groq"
    ...
    def test_save_settings_logs_correctly(self, caplog):
        ...
        assert "Settings updated: LLM Provider=OpenAI, Max Jobs=30" in caplog.text
...
class TestSettingsValidation:
    ...
    def test_invalid_llm_provider_fallback(self):
        """Test invalid LLM provider falls back to default."""
        st.session_state["llm_provider"] = "InvalidProvider"
        settings = load_settings()
        # Should still return the stored value, validation happens at UI level
        assert settings["llm_provider"] == "InvalidProvider"
...
class TestSettingsIntegration:
    ...
    def test_complete_settings_workflow(self):
        ...
        assert initial_settings["llm_provider"] == "OpenAI"
        ...
        assert updated_settings["llm_provider"] == "Groq"
